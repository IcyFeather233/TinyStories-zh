...
PROJECT_ROOT = './my_model'
DATASET_ROOT = '../../datasets/TinyStoriesZh'
...
def train_my_huggingface_wiki_tokenizer(corpus_file: str, max_train_line: int=None, vocab_size: int=40960,token_type: str='char') -> None:
    tokenizer_slow_save_path = PROJECT_ROOT + '/hf_tokenizer_slow/hf_bpe_tokenizer.json'
    tokenizer_fast_save_path = PROJECT_ROOT + '/hf_tokenizer'

    check_dir_exits(PROJECT_ROOT + '/hf_tokenizer_slow')
    ...
    def get_training_corpus(buffer_size: int=1000, chunk_len: int=2048) -> list:
        ...
        with open(corpus_file, 'r', encoding='utf-8') as f_read:
            ...
            for line in f_read:
                ...
                # 假设每行JSON有"text"字段，提取该字段内容
                if 'text' in json_data:
                    text_line = json_data['text']
                    cur_chunk_txt.append(text_line)
                    txt_len += len(text_line)
                    line_cnt += 1
...
if __name__ == '__main__':
    ...
    corpus_file = DATASET_ROOT + '/TinyStories-Zh-1M.jsonl'

    train_my_huggingface_wiki_tokenizer(corpus_file=corpus_file, token_type='byte') # token_type must be 'char' or 'byte'
